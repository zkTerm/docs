...
**Features**
- **Email OTP 2FA** - Alternative two-factor authentication method using email-based one-time passwords
- Multi-method 2FA support - Users can now choose between TOTP (authenticator app) or Email OTP
- Security email validation - 2FA email must differ from Google login email (validated frontend + backend)
- Action-based OTP tokens - Separate tokens for 'verify' vs 'disable' actions prevent replay attacks

**Email OTP Implementation**
- 8 backend endpoints: /status, /setup, /verify-setup, /verify-setup-manual, /send-otp, /verify-otp, /disable, /update
- SHA-256 hashed OTP codes with 10-minute expiry
- Decentralized on-chain storage via Solana Memo Program (same architecture as TOTP)
- State merging preserves existing TOTP data when adding Email OTP
- Session-based stateless verification with no PostgreSQL dependency

**Dashboard UI Improvements**
- Profile ID card with avatar image display
- Transparent card backgrounds showing VFX effects through
- Reusable DashboardCard component for consistent styling
- Improved spacing between header and explorer content

**Session Management**
- Migrated to cookie-based storage with memorystore
- Removed PostgreSQL dependency for session persistence
- Improved session status display on login page

**Security Architecture**
- Updated login page with Layer 1/Layer 2 security details
- Clear distinction: TOTP/Email OTP for daily 2FA, Hardware Wallet/Social Recovery for account recovery
- Enhanced security email storage with on-chain encryption

**Technical**
- server/twoFactorRoutes.ts - Extended with 8 Email OTP endpoints
- server/emailService.ts - New Resend API integration for OTP delivery
- client/src/components/TwoFactorSetup.tsx - Tabbed UI for TOTP vs Email OTP selection
- client/src/components/DashboardCard.tsx - Reusable transparent card component
- client/src/components/AuthenticatedDashboard.tsx - Profile card with avatar
