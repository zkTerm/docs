...
**Features**
- FULLY DECENTRALIZED zkAuth - 100% on-chain storage with NO PostgreSQL dependency
...
**Security**
- HMAC-based deterministic lookup keys with server salt prevents enumeration attacks
- Production safety: Server crashes immediately if ZKAUTH_LOOKUP_SALT env var is missing (prevents catastrophic key rotation)
- Development mode uses random session salt with clear warnings about data not persisting across restarts

**Technical**
- packages/zkauth/src/lookup.ts - deterministic key derivation with HMAC + server salt
- server/solanaProof.ts - storeZkAuthShareDecentralized() and searchZkAuthSharesByLookupKey() with pagination
...
- Set ZKAUTH_LOOKUP_SALT environment variable (64-character hex string) before deploying
